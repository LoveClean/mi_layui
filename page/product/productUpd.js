layui.use(['form', 'layer', 'layedit', 'upload'], function () {
    ...
    setTimeout(function () {
        $.ajax({
            url: $.cookie("tempUrl") + "product/selectByPrimaryKey?token=" + $.cookie("token") + "&id=" + $(".id").val(),
            type: "GET",
            success: function (result) {
                if (result.code === 0) {
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            if (result.code === 0) {
                                $.each(result.content,
                                    function (index, item) {
                                        item.categoryId === categoryId ? productCategory.append($('<option value=' + item.categoryId + ' selected>' + item.categoryName + '</option>')) : productCategory.append($('<option value=' + item.categoryId + '>' + item.categoryName + '</option>'));
                                    });
                                form.render();
                            } else {
                    ...
                    //加载富文本内容
                    layedit.setContent(editIndex, result.data.productArticleContent);
                } else {
    ...
    //创建一个编辑器
    const editIndex = layedit.build('productArticleContent', {
        height: 500,
        uploadImage: {
    ...
    form.on("submit(addNews)", function (data) {
        ...
        $.ajax({
            url: $.cookie("tempUrl") + "product/updateByPrimaryKeySelective?token=" + $.cookie("token"),
            type: "PUT",
